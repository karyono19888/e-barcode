...
class Tiang extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Produksi/M_tiang', 'record');
	}

	public function index()
	{
		$data['title']   = 'HRMS 1.0 | Tiang Barcode';
		$data['total_bc'] = $this->record->hitungJumlahGenerate();
		$data['generate'] = $this->record->t_generate_tiang();
		$this->load->view('template/v_header', $data);
		$this->load->view('template/v_sidebar');
		$this->load->view('produksi/v_bc_tiang', $data);
	...
	function mpdf($t_bc_id)
	{
		...
		$html = $this->load->view('produksi/v_tiang_single_pdf', ['bc' => $data], true);

		$mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => [200, 105]]);
		$mpdf->WriteHTML($html);
		$mpdf->Output();
	}
