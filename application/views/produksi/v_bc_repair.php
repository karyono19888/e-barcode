...
<div class="content-wrapper">
	...
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1><i class="fas fa-qrcode"></i> Repair Barcode</h1>
				</div>
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="#">Produksi</a></li>
						<li class="breadcrumb-item active">Repair Barcode</li>
					</ol>
				</div>
			</div>
		</div><!-- /.container-fluid -->
		...
		<div class="row">
				...
				<div class="card shadow">
					<form class="form-horizontal" id="form_generate" method="post" validation>
						<div class="card-header">
							<h5 class="m-0 font-weight-bold float-left">Form Kode Repiar</h5>
						</div>
						<div class="card-body">
							<input type="text" class="form-control" id="m_bc_jml" name="m_bc_jml" placeholder="Kode Repair" style="text-transform: uppercase" required>
						</div>
			...
			<div class="col-sm-9">
				<div class="card shadow mb-3">
					<div class="card-body">
						<form action="<?= base_url('Produksi/Repair/bc_all'); ?>" method="GET" target="_blank">
							<div class="form-row">
								<div class="col-sm-4">
									...
									?>
										<tr class="justify-content-center">
											...
											<td class="text-center">
												<img class="img-rounded" src="<?= ($row['t_bc_img'] == 'no-qrcode.png' ? base_url('assets/dist/img/no-qrcode.png') : base_url('assets/repair/' . $row['t_bc_img'])); ?>" style="width:100px;">
											</td>
											<td class="text-center">
												<a href="<?= base_url('Produksi/Repair/mpdf/' . $row['t_bc_id']); ?>" target="_blank" class="btn btn-default"><i class="fas fa-print"></i> Print</a>
											</td>
										</tr>
									<?php } ?>
								</tbody>
...
<script>
	...
	$('#m_bc_jml').inputmask("ASG500 22-99-999999")
	...
	$(document).on("click", "#tombol_generate", function() {
		if (validasi3()) {
			...
			$.ajax({
				type: 'POST',
				url: '<?= site_url('Produksi/Repair/generate') ?>',
				data: data,
				success: function(response) {
					...
					setTimeout(() => {
						window.location.assign('<?php echo site_url("Produksi/Repair") ?>');
					}, 1500);
				}
			});
		}
	...
	$(document).ready(function() {
		$("#start_bc").select2({
			theme: 'bootstrap4',
			ajax: {
				url: '<?= base_url() ?>Produksi/Repair/getkode_bc',
				type: "post",
				dataType: 'json',
				delay: 200,
	...
	$(document).ready(function() {
		$("#end_bc").select2({
			theme: 'bootstrap4',
			ajax: {
				url: '<?= base_url() ?>Produksi/Repair/getkode_bc2',
				type: "post",
				dataType: 'json',
				delay: 200,
				data: function(params) {
