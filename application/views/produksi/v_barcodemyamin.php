		...
		<div class="row">
			...
			<div class="col-sm-9">
				<div class="card shadow mb-3">
					<div class="card-body">
						<form action="<?= base_url('Produksi/BarcodeMyamin/bc_all'); ?>" method="GET" target="_blank">
							<div class="form-row">
								<div class="col-sm-4">
									...
									?>
										<tr class="justify-content-center">
											...
											<td class="text-center">
												...
												echo '<img src="data:image/png;base64,' . base64_encode($generator->getBarcode($row['t_bc_kode'], $generator::TYPE_CODE_128)) . '">';
												?>
											</td>
											<td class="text-center">
												<a href="<?= base_url('Produksi/BarcodeMyamin/mpdf/' . $row['t_bc_id']); ?>" target="_blank" class="btn btn-default"><i class="fas fa-print"></i> Print</a>
											</td>
										</tr>
									<?php } ?>
								</tbody>
...
<script>
	...
	$(document).on("click", "#tombol_generate", function() {
		if (validasi3()) {
			...
			$.ajax({
				type: 'POST',
				url: '<?= site_url('Produksi/BarcodeMyamin/generate') ?>',
				data: data,
				success: function(response) {
					...
					setTimeout(() => {
						window.location.assign('<?php echo site_url("Produksi/BarcodeMyamin") ?>');
					}, 1500);
				}
			});
		}
	...
	$(document).ready(function() {
		$("#start_bc").select2({
			theme: 'bootstrap4',
			ajax: {
				url: '<?= base_url() ?>Produksi/BarcodeMyamin/getkode_bc',
				type: "post",
				dataType: 'json',
				delay: 200,
	...
	$(document).ready(function() {
		$("#end_bc").select2({
			theme: 'bootstrap4',
			ajax: {
				url: '<?= base_url() ?>Produksi/BarcodeMyamin/getkode_bc2',
				type: "post",
				dataType: 'json',
				delay: 200,
				data: function(params) {
