<div class="row">
		...
		<div class="card">
			...
			<div class="card-body">
				<dl class="row">
					...
					<dd class="col-sm-8"><?= $data['m_line_kode']; ?></dd>
					<dt class="col-sm-3">Nama</dt>
					<dd class="col-sm-8"><?= $data['m_line_nama']; ?></dd>
					<dt class="col-sm-3">Status</dt>
					<dd class="col-sm-8">
						<?php if ($data['m_line_status'] == "Aktif") : ?>
							<span class="badge badge-pill badge-primary"><?= $data['m_line_status']; ?></span>
						<?php else : ?>
							<span class="badge badge-pill badge-primary"><?= $data['m_line_status']; ?></span>
						<?php endif; ?>
					</dd>
	...
	<div class="col-sm-3">
		<div class="card">
			<div class="card-body">
				<button type="button" class="btn btn-block bg-gradient-warning Edit" data-id="<?= $data['m_line_id']; ?>">Edit</button>
				<button type="button" data-id="<?= $data['m_line_id']; ?>" class="btn btn-block bg-gradient-danger" id="TombolDelete">Delete</button>
...
<script>
	...
	$(document).on("click", ".Edit", function() {
		...
		$.ajax({
			type: "POST",
			url: "<?= site_url('Master/Line/ShowEditData') ?>",
			data: {
				id: id
			},
			success: function(response) {
